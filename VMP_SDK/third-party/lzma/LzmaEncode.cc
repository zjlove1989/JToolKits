...
#define BSR2_RET(pos, res)                                                      \
    {                                                                           \
        UInt32 zz = (pos < (1 << (kNumLogBits + 6))) ? 6 : 6 + kNumLogBits - 1; \
        res = p->g_FastPos[pos >> zz] + (zz * 2);                               \
    }
...
static UInt32 GetOptimum(CLzmaEnc* p, UInt32 position, UInt32* backRes)
{
...

    {
...

        {
            ...
            p->opt[1].price = GET_PRICE_0(p->isMatch[p->state][posState]) + (!IsCharState(p->state) ? LitEnc_GetPriceMatched(probs, curByte, matchByte, p->ProbPrices) : LitEnc_GetPrice(probs, curByte, p->ProbPrices));
        }
            ...
            for (repIndex = 0; repIndex < LZMA_NUM_REPS; repIndex++) {
                ...
                /* if (_maxMode) */
                {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        UInt32 curAndLenCharPrice = price + p->repLenEnc.prices[posState][lenTest - 2] + GET_PRICE_0(p->isMatch[state2][posStateNext]) + LitEnc_GetPriceMatched(LIT_PROBS(position + lenTest, data[lenTest - 1]), data[lenTest], data2[lenTest], p->ProbPrices);
            ...
            for (lenTest = /*2*/ startLen;; lenTest++) {
                ...
                if (/*_maxMode && */ lenTest == matches[offs]) {
                    ...
                    if (lenTest2 >= 2) {
                        ...
                        UInt32 posStateNext = (position + lenTest) & p->pbMask;
                        UInt32 curAndLenCharPrice = curAndLenPrice + GET_PRICE_0(p->isMatch[state2][posStateNext]) + LitEnc_GetPriceMatched(LIT_PROBS(position + lenTest, data[lenTest - 1]), data[lenTest], data2[lenTest], p->ProbPrices);
                        state2 = kLiteralNextStates[state2];
...
static UInt32 GetOptimumFast(CLzmaEnc* p, UInt32* backRes)
{
    ...
    *backRes = (UInt32)-1;
    ...
    if (p->longestMatchLength >= 2) {
        ...
        if ((p->longestMatchLength >= mainLen && newDistance < mainDist) || (p->longestMatchLength == mainLen + 1 && !ChangePair(mainDist, newDistance)) || (p->longestMatchLength > mainLen + 1) || (p->longestMatchLength + 1 >= mainLen && mainLen >= 3 && ChangePair(newDistance, mainDist)))
            return 1;
    }
