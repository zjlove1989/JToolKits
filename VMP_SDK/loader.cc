...
static void* LoaderAlloc(int size)
{
#ifdef VMP_GNU
    return malloc(size);
...
__declspec(noinline) const wchar_t* LoaderFindFileVersion(const uint8_t* ptr, size_t data_size)
{
    ...
    for (size_t i = 0; i < data_size; i++) {
        if (data_size >= 13) {
            if (data[i + 0] == L'F' && data[i + 1] == L'i' && data[i + 2] == L'l' && data[i + 3] == L'e' && data[i + 4] == L'V' && data[i + 5] == L'e' && data[i + 6] == L'r' && data[i + 7] == L's' && data[i + 8] == L'i' && data[i + 9] == L'o' && data[i + 10] == L'n' && data[i + 11] == 0 && data[i + 12] == 0)
                return data + i + 13;
        }
        if (data_size >= 15) {
            if (data[i + 0] == L'P' && data[i + 1] == L'r' && data[i + 2] == L'o' && data[i + 3] == L'd' && data[i + 4] == L'u' && data[i + 5] == L'c' && data[i + 6] == L't' && data[i + 7] == L'V' && data[i + 8] == L'e' && data[i + 9] == L'r' && data[i + 10] == L's' && data[i + 11] == L'i' && data[i + 12] == L'o' && data[i + 13] == L'n' && data[i + 14] == 0)
                return data + i + 15;
        }
    }
    return NULL;
    ...
    *dst = 0;
    ...
    if (tNtRaiseHardError* raise_hard_error = reinterpret_cast<tNtRaiseHardError*>(LoaderGetProcAddress(ntdll, reinterpret_cast<const char*>(FACE_NT_RAISE_HARD_ERROR_NAME), true))) {
        ...
        ULONG_PTR params[4] = {
            ...
            (ULONG_PTR)&title_str,
            (
                (ULONG)ResponseButtonOK | (type == mtUnregisteredVersion ? IconWarning : IconError)),
            INFINITE
        };
