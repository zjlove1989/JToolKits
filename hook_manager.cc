...
{
    void* res = NULL;
    void* api_address = GetProcAddress(dll, api_name);
    if (api_address) {
        Begin();
...
bool HookManager::UnhookAPI(void* handler)
{
    ...
    for (size_t i = 0; i < api_list_.size(); i++) {
        ...
        if (hooked_api->old_handler() == handler) {
            api_list_.erase(api_list_.begin() + i);
            hooked_api->Unhook();
            delete hooked_api;
            res = true;
            break;
...
size_t MoveCode(byte*& pSrcOriginal, byte* pDst, size_t nNeedBytes)
{
    ...
    while (nBytes < nNeedBytes) {
        if (!GetInstructionSize(pSrc, ii))
            return 0; // error
        if (nBytes == 0 && ii.cmdType == CT_JMP && ii.dwSize - ii.dwRelOffset == 1) { // this is a short jump and we need to follow it
